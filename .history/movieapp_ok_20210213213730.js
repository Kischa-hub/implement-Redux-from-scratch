...
const render = (movietitle = "", moviecateogry = "") => {
  ...
  switch (lastAction.type) {
    case "UNDO":
      current_store.push(store.getState().undoableReducer.future);
    //return console.log("FUTRURE", current_store);
    case "REDO":
      current_store.push(store.getState().undoableReducer.present);
    // return console.log("PRESENT", current_store);
    default:
      document.body.innerHTML = "";
      current_store.push(store.getState().movies);
  }
  current_store.map((m) => {
