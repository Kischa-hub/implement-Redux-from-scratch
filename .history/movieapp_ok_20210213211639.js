...
const render = (movietitle = "", moviecateogry = "") => {
  ...
  switch (lastAction) {
    case "UNDO":
      return (current_store = store.getState().undoableReducer.future);
    // console.log("FUTRURE", current_store);
    case "REDO":
      return (current_store = store.getState().undoableReducer.present);
    // console.log("PRESENT", current_store);
    default:
      return (current_store = store.getState().movies);
  }
  current_store.map((m) => {
