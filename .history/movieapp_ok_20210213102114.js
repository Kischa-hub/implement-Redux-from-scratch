...
const render = (movietitle = "", moviecateogry = "") => {
  movietitle = store.getState().movies[0].name;
  let div = document.createElement("div");
  div.classList.add("recipe");

  let h1 = document.createElement("h1");
  h1.textContent = movietitle;

  let h3 = document.createElement("h3");
  h3.textContent = moviecateogry;
  ...
  store.getState().movies.map( movietitle = store.getState().movies[0].name;
  let div = document.createElement("div");
  div.classList.add("recipe");
...
store.subscribe(() => {
  ...
  //   .getState()
  //   .movies.map((m) => render(m.name, m.details));
  render();
  //console.log(store.getState().movies.map((m)=>m.name))
  //{names.map(name => <li key={name}> {name} </li>)}</ul>
  // document.body.innerText = store
  //   .getState()
  //   .movies.map((m) => m.name + " " + m.details);
});
